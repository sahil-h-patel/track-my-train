...
// getTrainSchedule
async function getTrainScheduleForStation(station: string): Promise<StationTrainSchedule> {
    try {
        const body = new FormData()
...
// getStationSchedule
async function getStationSchedule(station?: string): Promise<StationSchedule>{
    try {
        const body = new FormData()
        body.append('token', token);
